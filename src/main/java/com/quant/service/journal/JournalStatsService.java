...
public class JournalStatsService {
  ...
  public JournalStatsDTO stats(JournalTrade.Mode mode) {
    List<JournalTrade> closed =
        mode == null ? repo.findAllClosedOrdered() : repo.findClosedByMode(mode);
    ...
    BigDecimal ev =
        winRate.multiply(avgWin).add(lossRate.multiply(avgLoss)).setScale(4, RoundingMode.HALF_UP);
