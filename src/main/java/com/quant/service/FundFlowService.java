 ...
 * 东方财富板块资金流拉取服务。
 *
 * <p>数据来源：https://push2.eastmoney.com/api/qt/clist/get
 * 字段映射：
 * <ul>
 *   <li>f12 = 板块代码</li>
 *   <li>f14 = 板块名称</li>
...
public class FundFlowService {
    ...
    private static String urlInflow(int page, int size) {
        return "https://" + EM_HOST + "/api/qt/clist/get?pn=" + page + "&pz=" + size + "&po=1&np=1"
            + "&fltt=2&invt=2&fid=f62&fs=m:90+t:2+f:!50"
            + "&fields=" + FIELDS + "&ut=b2884a39322259f484a851477f2a9a86";
    }
    // 流出（po=0, fid=f62 asc）：主力净流出最多的板块（负值最多）
    private static String urlOutflow(int page, int size) {
        return "https://" + EM_HOST + "/api/qt/clist/get?pn=" + page + "&pz=" + size + "&po=0&np=1"
            + "&fltt=2&invt=2&fid=f62&fs=m:90+t:2+f:!50"
            + "&fields=" + FIELDS + "&ut=b2884a39322259f484a851477f2a9a86";
    }
    ...
    private final AtomicReference<CachedResult> cache = new AtomicReference<>(new CachedResult(List.of(), LocalDateTime.MIN));
    ...
    private List<SectorFundFlow> fetchOnePage(String url, boolean isInflow) {
        try {
            String body = httpClient.getWithReferer(url, "https://data.eastmoney.com/", StandardCharsets.UTF_8);
    ...
    private String yuan(Map<String, Object> m, String key) {
        ...
        if (v instanceof Number n) {
            ...
            try { val = Double.parseDouble(v.toString()); }
    ...
    private FundFlowData buildResponse(CachedResult cached) {
        ...
        List<SectorFundFlow> sorted = all.stream()
            .sorted((a, b) -> {
