...
class JournalControllerTest {
  ...
  void list_returnsPage() throws Exception {
    ...
    mvc.perform(get("/api/journal/trades")).andExpect(status().isOk());
  }

  @Test
  @WithMockUser
  void delete_trade_returnsNoContent() throws Exception {
    mvc.perform(delete("/api/journal/trades/1")).andExpect(status().isNoContent());
  }
  ...
  void get_openList_returnsList() throws Exception {
    when(service.listOpen())
        .thenReturn(
            java.util.List.of(JournalTradeDTO.builder().id(1L).stockCode("600519").build()));
