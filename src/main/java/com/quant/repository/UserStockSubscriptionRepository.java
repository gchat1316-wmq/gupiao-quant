...
public interface UserStockSubscriptionRepository
  ...
  @Query(
      "SELECT s FROM UserStockSubscription s WHERE s.enabled = true "
  ...
  @Modifying
  @Query(
      "UPDATE UserStockSubscription s SET s.alertBuyTriggeredAt = NULL, "
  ...
  int clearAllTriggeredAt(@Param("userId") Long userId, @Param("stockCode") String stockCode);
}
