...
class JournalStatsServiceTest {
  ...
  void equityCurve_cumulativeR() {
    when(repo.findAllClosedOrdered())
        .thenReturn(
            List.of(
  ...
  void rDistribution_sevenBuckets() {
    ...
    assertEquals(
        1L, bk.stream().filter(b -> b.getLabel().equals("<-2R")).findFirst().get().getCount());
    assertEquals(
        1L, bk.stream().filter(b -> b.getLabel().equals("-2~-1R")).findFirst().get().getCount());
    assertEquals(
        1L, bk.stream().filter(b -> b.getLabel().equals(">3R")).findFirst().get().getCount());
  }
