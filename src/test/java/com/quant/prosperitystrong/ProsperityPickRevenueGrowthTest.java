...
/**
 * 回归测试: "龙头候选"表格的"近4季净利率"列改成"近3季度营收增速"。 DTO 必须把 entity.revenueYoyMin4q(实际存的是 revenueYoyMin3q 数据)
 * 暴露出来。
 */
@SpringBootTest
@ActiveProfiles("test")
class ProsperityPickRevenueGrowthTest {
  ...
  void pickDailyDTO_exposes_revenueYoyMin3q() throws Exception {
    ...
    // 调用 toPickDTO
    Method m =
        ProsperityStrongPipelineService.class.getDeclaredMethod(
    ...
    assertEquals(
        0, new BigDecimal("12.34").compareTo(rev), "revenueYoyMin3q 应映射 entity.revenueYoyMin3q");
  }

  /**
   * 用反射读 DTO 字段 — 因为 Lombok @Builder 生成的 getter 名称可能因字段名而变。 优先 revenueYoyMin3q, 退而求其次
   * revenueYoyMin4q。
   */
  private BigDecimal readRevenue(PickDailyDTO dto) {
    for (String name : new String[] {"getRevenueYoyMin3q", "getRevenueYoyMin4q"}) {
      try {
        ...
        fail("反射调用 " + name + " 失败: " + e.getMessage());
      }
    }
    return null;
  }
