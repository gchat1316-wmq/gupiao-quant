 ...
 * 每周机会点（3×3 卡片）槽位。
 *
 * <p>每个 pool_type 固定 9 个 slot（slotIndex 0~8）。空槽 stockCode=NULL。 估值水平 (level) 不入库，读取时按 stockCode
 * 实时联动 invest_stock_pool 算出。
 */
@Getter
@Setter
@Entity
@Table(
    name = "invest_weekly_opportunity_slot",
    uniqueConstraints =
...
public class InvestWeeklyOpportunitySlot {
  ...
  /**
   * 用户在 9 格编辑页手工填的股票名（与 stockPool 联动的 stockName 独立）。 用途：stockCode 不在池中时，buildSlots 联动不到名称，此时用
   * userStockName 兜底显示。
   */
