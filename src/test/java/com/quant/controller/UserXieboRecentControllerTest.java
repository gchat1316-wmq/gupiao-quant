...
class UserXieboRecentControllerTest {
  ...
  @MockBean XieboRecentSubscriptionService subscriptionService;
  ...
  void list_returnsServiceResult() throws Exception {
    ...
    mvc.perform(get("/api/me/recent/subscriptions").header("Authorization", "Bearer " + userToken))
  ...
  void upsert_callsService() throws Exception {
    when(subscriptionService.upsert(eq(7L), eq("600519"), any()))
        .thenReturn(
            UserSubscriptionDto.builder().id(42L).stockCode("600519").enabled(true).build());
