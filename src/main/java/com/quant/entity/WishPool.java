 ...
 *
 * <p>流程： 1) 用户提交 → {@code status=PENDING, display=false}，异步推飞书 webhook 通知到管理员。 2) 管理员在后台回复（{@code
 * reply} + status=REPLIED, reply_by=admin, reply_at=now}）。 3) 管理员可勾选 {@code display=true}，前台右下角
 * {@code GET /api/wishes/public} 会轮播这些已 display=true 且有回复的条目。
 *
