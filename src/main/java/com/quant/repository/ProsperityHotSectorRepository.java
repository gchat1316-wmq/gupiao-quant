...
public interface ProsperityHotSectorRepository extends JpaRepository<ProsperityHotSector, Integer> {
    ...
    /**
     * 只查必要字段（不含 aiNarrative 这个 TEXT）。
     * sectors 列表接口使用这个避免拉长文本导致 InnoDB off-page 读。
     * aiNarrative 另行单查或详情接口取。
     */
    @Query("SELECT new com.quant.service.prosperitystrong.SectorSummaryDTO(" +
    ...
    List<com.quant.service.prosperitystrong.SectorSummaryDTO> findSummaryBySnapDate(@Param("snapDate") LocalDate snapDate);

    /**
     * 单独批量拉 aiNarrative 字段（带 id 便于组装）。
     * 列表页用上 SectorSummaryDTO 后，仅取这 N 个 id 的 TEXT，避开 Entity 全字段投影。
     */
    @Query(value = "SELECT id, ai_narrative FROM prosperity_hot_sector WHERE id IN (:ids)", nativeQuery = true)
