 ...
 *
 * <p>场景：用户在"科技风投 · 10 倍 PS 法"卡片里手动输入市值和 Y0/Y1/Y2 营收，
 * 前端调这个端点拿回 verdict + commentary + 三行明细。
 *
...
class ValuationControllerTest {
    ...
    class CalcPs10 {
        ...
        void returnsVerdictAndRows() throws Exception {
            Ps10ValuationService.Ps10Result stub = new Ps10ValuationService.Ps10Result(
                    true, "合理",
            ...
            String body = """
                    ...
                    """;
            mvc.perform(post("/api/valuation/ps10")
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(body))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.verdict").value("合理"))
                    .andExpect(jsonPath("$.commentary").value(org.hamcrest.Matchers.containsString("113.7 亿")))
        ...
        void worksWithoutNetMargin() throws Exception {
            Ps10ValuationService.Ps10Result stub = new Ps10ValuationService.Ps10Result(
                    ...
                    null, null, new java.math.BigDecimal("100")
            );
            when(ps10ValuationService.evaluateFromInputs(
