 ...
 *
 * 流程:
 *   1. 前端调用 GET /api/tdx/auth/status — 检查是否已授权
 *   2. 未授权时调用 GET /api/tdx/auth/start — 返回 authorize URL, 后端生成 PKCE+state 写入 cache
 *   3. 前端引导用户在浏览器打开 authorize URL, 用户登录腾讯账号并授权
 *   4. TDX 回调 GET /api/tdx/auth/callback?code=xxx&state=xxx
 *   5. 后端用 code + code_verifier 调 /token 换 access_token + refresh_token, 缓存到 token.json
 *   6. 后续调 TDX MCP 工具 (wenda_report_query 等) 会自动用 cache 里的 access_token
 */
@RestController
...
public class TdxAuthController {
    ...
    public Map<String, Object> logout() {
        ...
        try {
            java.nio.file.Files.deleteIfExists(java.nio.file.Path.of(
