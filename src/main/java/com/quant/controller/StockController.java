...
public class StockController {
  ...
  public QueryResultDTO queryFinancial(
  ...
  /**
   * 修复 trade_stock_financial 表中年报/Q3 累计营收 -> 单季营收 Q3 = Q3累计 - Q2；Q4 = annual - Q1 - Q2 - Q3 注意：Q4
   * 已严重损坏（如药明康德-1357亿）的行，公式 Q4=Q4-Q1-Q2-Q3 会越修越差 需用 /admin/force-fix-field 手动修复
   */
  ...
  public ResponseEntity<?> clearYoyCache(
      @RequestParam String code, @RequestParam(required = false) Integer year) {
    var jdbc = new org.springframework.jdbc.core.JdbcTemplate(dataSource);
    String sql;
    Object[] params;
    if (year != null) {
      sql =
          "UPDATE trade_stock_financial SET revenue_yoy = NULL WHERE stock_code = ? AND YEAR(report_date) = ?";
  ...
  public ResponseEntity<?> forceFixField(
    ...
    int updated =
        jdbc.update(
            "UPDATE trade_stock_financial SET "
                + col
