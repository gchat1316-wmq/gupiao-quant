 ...
 * Step 2: 龙头识别
 *
 * <p>在板块成分股内按 (年度涨幅 + 5日涨幅 + 换手率) 加权评分, 并剔除 ST / 次新股(<1年)。不使用停牌字段过滤,该数据源不稳定。
 *
 * <p>为简化首阶段,从 TradeStockBasic.sectorNames 字段中模糊匹配板块名称, 找出该板块内的成分股进行评分。
 */
@Slf4j
@Component
...
public class LeaderIdentifier {
  ...
  public List<ProsperityLeaderCandidate> identify(LocalDate snapDate, ProsperityHotSector sector) {
    ...
    Map<String, AStockDataQuoteService.QuoteSnapshot> latestQuoteMap =
    ...
    for (TradeStockDaily d :
        dailyRepo.findLatestByStockCodes(
            realMembers.stream().map(TradeStockBasic::getStockCode).toList())) {
    ...
    for (TradeStockDaily d :
        dailyRepo.findFirstAfterDateByStockCodes(
    ...
    for (TradeStockBasic basic : realMembers) {
      String codeKey =
          basic.getStockCode() == null ? "" : basic.getStockCode().trim().toUpperCase(Locale.ROOT);
      ...
      BigDecimal ytdChange =
          ytdChangeByLatestPrice(latestPrice, yearStartQuotes.get(basic.getStockCode()));
      BigDecimal turnover = latestKline.getTurnoverRate();
      // 简化的 5 日涨幅:用最近6条记录里的第一/最后一条
      List<TradeStockDaily> last6 =
          dailyRepo.findTop6ByStockCodeOrderByTradeDateDesc(basic.getStockCode());
    ...
    scored.sort(
        Comparator.comparing(
            ProsperityLeaderCandidate::getLeaderScore,
  ...
  private MemberLookup lookupMembers(ProsperityHotSector sector) {
    ...
    if (canUseEastMoneyMembers(sector)) {
      try {
        List<TradeStockBasic> membersByCode =
            findMembersByEastMoneySectorCode(sector.getSectorCode());
  ...
  private List<TradeStockBasic> findMembersByEastMoneySectorCode(String sectorCode)
      throws Exception {
  ...
  List<String> fetchEastMoneyMemberCodes(String sectorCode) throws Exception {
    ...
    ResponseEntity<String> response =
        rest.exchange(URI.create(url), HttpMethod.GET, new HttpEntity<>(headers), String.class);
  ...
  private List<TradeStockBasic> findMembersByAliases(String sectorName) {
    ...
    if (sectorIndexCache.getStatus() != null && sectorIndexCache.getStatus().stockCount() > 0) {
      return sectorIndexCache.getOrLoadByKeywords(
          keywords,
          () -> {
  ...
  /** 轻量入口：直接传 sectorName，避免上游传 Entity 走 sectors 列表接口。 sectors() 现已不拉 Entity，本接口仅取 sectorName 用。 */
  ...
  private BigDecimal ytdChange(TradeStockDaily latest, TradeStockDaily base) {
    ...
    return cur.subtract(old).divide(old, 6, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(100));
  }
  ...
  public record MemberStats(
      int matchedMemberCount, int quotedMemberCount, String diagnosticMessage) {}
}
