...
public class LegendDTO {
    ...
    public static java.util.List<RiskRewardPreset> presets() {
        return java.util.Arrays.asList(
                RiskRewardPreset.builder().label("1:1 保守").ratioValue(BigDecimal.valueOf(1)).breakEvenWinRate(new java.math.BigDecimal("0.5000")).note("盈亏平衡需 50% 胜率,业余线").build(),
                RiskRewardPreset.builder().label("1:2 标准").ratioValue(BigDecimal.valueOf(2)).breakEvenWinRate(new java.math.BigDecimal("0.3333")).note("盈亏平衡需 ~33% 胜率,具备统计优势").build(),
                RiskRewardPreset.builder().label("1:3 Minervini").ratioValue(BigDecimal.valueOf(3)).breakEvenWinRate(new java.math.BigDecimal("0.2500")).note("Mark Minervini 的最低进场门槛").build(),
                RiskRewardPreset.builder().label("1:5 进取").ratioValue(BigDecimal.valueOf(5)).breakEvenWinRate(new java.math.BigDecimal("0.1667")).note("盈亏平衡只需 ~17%,但回撤放大").build(),
                RiskRewardPreset.builder().label("1:10 大波段").ratioValue(BigDecimal.valueOf(10)).breakEvenWinRate(new java.math.BigDecimal("0.0909")).note("Livermore 风格的「让利润奔跑」档").build()
        );
    }
