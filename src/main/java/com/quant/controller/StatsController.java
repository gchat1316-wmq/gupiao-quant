...
public class StatsController {
  ...
  /**
   * 前端每次访问页面时调用，静默处理。 POST /api/stats/page-view Body: { pagePath, sessionId } （可选: userId,
   * userAgent）
   */
  ...
  /** 近 N 天每日概况（默认 30 天） GET /api/stats/daily-overview?days=30 */
  @GetMapping("/daily-overview")
  public List<DailyOverviewDTO> dailyOverview(@RequestParam(defaultValue = "30") int days) {
    return statsService.getDailyOverview(Math.min(days, 90));
  }

  /** 指定日期完整统计 GET /api/stats/daily?date=2026-06-28 */
  @GetMapping("/daily")
  ...
  /** 单个页面近 N 天访问趋势 GET /api/stats/page-trend?pagePath=/gp/invest.html&days=14 */
  @GetMapping("/page-trend")
  public List<?> pageTrend(
      @RequestParam String pagePath, @RequestParam(defaultValue = "14") int days) {
