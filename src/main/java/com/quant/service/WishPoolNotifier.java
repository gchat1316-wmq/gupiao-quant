...
/**
 * 异步推飞书 webhook — 拆出来主要是为了走 Spring @Async 代理（同 Bean 内自调失效）。 新留言入库后由 {@link WishPoolService}
 * 异步触发,失败仅记日志,不阻塞用户提交。
 */
@Slf4j
@Component
public class WishPoolNotifier {
  ...
  @Autowired
  public WishPoolNotifier(
      NotificationProperties properties, RestTemplateBuilder restTemplateBuilder) {
    this(
        properties,
        restTemplateBuilder
  ...
  public void notifyNewWish(WishPool wish) {
    ...
    StringBuilder text =
        new StringBuilder("【投资助手·许愿池】新留言 #")
            .append(wish.getId())
            .append("\n来源：")
            .append(wish.getPage() == null ? "未知页面" : wish.getPage())
            .append("\n愿望：")
            .append(wish.getWish());
    ...
    try {
      ResponseEntity<String> response =
          restTemplate.postForEntity(
              cfg.getWebhookUrl(), new HttpEntity<>(payload, headers), String.class);
      if (response.getStatusCode().is2xxSuccessful()) {
        log.info("wish pool notify ok: id={}", wish.getId());
      } else {
        log.warn(
            "wish pool notify non-2xx: id={} status={}", wish.getId(), response.getStatusCode());
      }
    } catch (Exception e) {
