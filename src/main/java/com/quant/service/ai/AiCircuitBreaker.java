 ...
 * 简易 AI 服务熔断器。
 *
 * <p>痛点: MiniMax API key 在 2026-06-26 已过期 (401 authentication_error)， 板块叙事流水线每次都先调 MiniMax，等 90s
 * 超时后才回退 SenseNova， 5 个板块 × 90s = 7.5 分钟白等。
 *
 * <p>方案: 累计 N 次 401/403 后熔断 30 分钟，期间直接跳过该 provider。 仅对 authentication_error / 401 / 403 触发熔断，
 * 业务错误（400 bad_request / 500 server_error）不熔断。
 */
@Slf4j
@Component
public class AiCircuitBreaker {
  ...
  public boolean recordAuthFailure(String provider) {
    ...
    if (count >= FAILURE_THRESHOLD) {
      ...
      openUntil.set(until);
      log.error("AI provider [{}] 连续 {} 次鉴权失败，已熔断至 {}。期间将直接跳过该 provider。", provider, count, until);
      return true;
    }
