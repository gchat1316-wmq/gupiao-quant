 ...
 *
 * <ul>
 *   <li>ProcessBuilder("curl") → 共享 Java HttpClient
 *   <li>逐只 URL → 新浪批量 URL（list=sh600000,sz000001, ...，一次最多 50/批）
 *   <li>多批并发
 * </ul>
 */
@Slf4j
...
public class SinaRealtimeQuoteService {
  ...
  public Map<String, TechAiQuoteSnapshot> fetch(Collection<String> codes) {
    ...
    List<CompletableFuture<Map<String, TechAiQuoteSnapshot>>> futures =
        new ArrayList<>(batches.size());
    for (List<String> batch : batches) {
      futures.add(
          CompletableFuture.supplyAsync(() -> fetchBatch(batch), quoteHttpClient.executor()));
    }
  ...
  private Map<String, TechAiQuoteSnapshot> fetchBatch(List<String> projectCodes) {
    ...
    String sinaCodes = projectCodes.stream().map(this::toSinaCode).collect(Collectors.joining(","));
