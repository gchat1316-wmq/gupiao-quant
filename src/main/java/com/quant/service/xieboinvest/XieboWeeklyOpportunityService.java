 ...
 *
 * 不变量：
 * 1. 每个 poolType 固定返回 9 个 slot（不足补空）
 * 2. slot 顺序按 slotIndex 升序
 * 3. update() 是「全量替换」语义（事务内先删后插）
 * 4. stockName 实时联动 invest_xiebo_watchlist 补全
 */
@Slf4j
@Service
...
public class XieboWeeklyOpportunityService {
    ...
    @Transactional
    public List<XieboWeeklyOpportunitySlotDTO> update(String poolType, XieboWeeklyOpportunityUpdateRequest req) {
        validatePoolType(poolType);
        validateSlots(req);
    ...
    private List<XieboWeeklyOpportunitySlotDTO> buildSlots(String poolType,
                                                           List<XieboWeeklyOpportunitySlot> rows,
        ...
        for (int i = 0; i < SLOTS_PER_POOL; i++) {
            ...
            if (s == null || s.getStockCode() == null) {
                out.add(XieboWeeklyOpportunitySlotDTO.builder()
                        .poolType(poolType)
                        .slotIndex(i)
                        .build());
            } else {
    ...
    private void validateSlots(XieboWeeklyOpportunityUpdateRequest req) {
        ...
        if (req.getSlots().size() != SLOTS_PER_POOL) {
            throw new IllegalArgumentException("slots 数量必须等于 " + SLOTS_PER_POOL +
                    "，实际 " + req.getSlots().size());
        }
        ...
        for (XieboWeeklyOpportunityUpdateRequest.SlotItem item : req.getSlots()) {
            if (item.getSlotIndex() == null
                ...
                throw new IllegalArgumentException("slotIndex 越界（必须在 0~" +
                        (SLOTS_PER_POOL - 1) + "），实际 " + item.getSlotIndex());
            }
