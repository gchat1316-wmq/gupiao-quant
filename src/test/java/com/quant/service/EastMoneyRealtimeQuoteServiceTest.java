 ...
 * <ol>
 *   <li>N 个 code 全部被并发请求（不串行阻塞）</li>
 *   <li>主域失败 → 切备用域；备用域成功 → 返回结果</li>
 *   <li>主备都失败 → 返 null（不抛异常）</li>
 * </ol>
 *
...
class EastMoneyRealtimeQuoteServiceTest {
    ...
    void fetchConcurrently() {
        ...
        doAnswer(inv -> {
            ...
            return "{\"rc\":0,\"data\":{\"f43\":1000,\"f57\":\"" + code + "\",\"f86\":1700000000}}";
        }).when(quoteHttpClient).getUtf8(anyString());
    ...
    void fetchFallsBackToBackupHost() {
        ...
        doReturn(null).when(quoteHttpClient).getUtf8(org.mockito.ArgumentMatchers.contains("push2.eastmoney.com"));
        doReturn(json).when(quoteHttpClient).getUtf8(org.mockito.ArgumentMatchers.contains("push2delay.eastmoney.com"));
