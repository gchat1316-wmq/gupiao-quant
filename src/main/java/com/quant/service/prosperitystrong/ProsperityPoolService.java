 ...
 *
 * <p>与 {@code InvestService.addToPool}（龙江投资股票池）独立： 本服务只服务于"热点选股"模块的短线/波段池子，pool_type 语义、字段模型都不同。
 *
 * <p>幂等性：同股票重复入池 = 累加 {@code poolCount} + 更新 {@code last_added_at} / {@code last_snap_date} + 在
 * {@code memo} 末尾追加新推荐理由。绝不抛"已存在"错误， 因为流水线按日跑，重复触发是常态。
 */
@Slf4j
@Service
...
public class ProsperityPoolService {
  ...
  public Map<String, Object> promote(String stockCode, LocalDate snapDate, Long ownerId) {
    ...
    ProsperityPickDaily pick =
        pickRepo
            .findBySnapDateAndStockCode(snapDate, stockCode)
    ...
    log.info(
        "龙头候选入池: code={} ownerId={} snapDate={} isNew={} poolCount={}",
        stockCode,
        ownerId,
        snapDate,
        isNew,
  ...
  public List<ProsperityStockPool> list(Long ownerId) {
    List<ProsperityStockPool> result =
        new ArrayList<>(poolRepo.findByOwnerIdOrderByLastAddedAtDesc(ownerId));
