 ...
 *
 * <p>策略：用真实 JwtTokenProvider 生成 JWT，放在请求头 Authorization: Bearer <token> 让 JwtAuthFilter 走完整的
 * token→User→UserPrincipal 链路。
 *
 * <p>公开接口（permitAll）：send-code / verify-code / login / login-code / wechat-* 需要认证（anyRole）：/me /
 * set-password ADMIN 专属：/admin/*
 */
@WebMvcTest(
...
class AuthControllerTest {
  ...
  class SendCode {
    ...
    void serviceThrowsReturns400() throws Exception {
      when(authService.sendCode(anyString(), anyString())).thenThrow(new RuntimeException("频率限制"));
  ...
  class ResetPasswordByEmail {
    ...
    void resetSuccess() throws Exception {
      doNothing()
          .when(authService)
          .resetPasswordByEmail(eq("a@b.com"), eq("123456"), eq("NewPwd!"), anyString());

      mvc.perform(
              post("/api/auth/reset-password-email")
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(
                      "{\"email\":\"a@b.com\",\"code\":\"123456\",\"newPassword\":\"NewPwd!\"}"))
          .andExpect(status().isOk())
    ...
    void userNotFoundReturns400() throws Exception {
      doThrow(new RuntimeException("用户不存在"))
          .when(authService)
          .resetPasswordByEmail(anyString(), anyString(), anyString(), anyString());
  ...
  class ResetPasswordBySms {
    ...
    void resetSuccess() throws Exception {
      doNothing()
          .when(authService)
          .resetPasswordBySms(eq("13800138000"), eq("123456"), eq("NewPwd!"), anyString());

      mvc.perform(
              post("/api/auth/reset-password-sms")
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(
                      "{\"phone\":\"13800138000\",\"code\":\"123456\",\"newPassword\":\"NewPwd!\"}"))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.message").value("密码已重置"));
    }
    ...
    void wrongCodeReturns400() throws Exception {
      doThrow(new RuntimeException("验证码错误或已过期"))
          .when(authService)
          .resetPasswordBySms(anyString(), anyString(), anyString(), anyString());
  ...
  class Me {
    ...
    void managerMeReturnsManagerInfo() throws Exception {
      mvc.perform(get("/api/auth/me").header("Authorization", "Bearer " + managerToken))
          .andExpect(status().isOk())
  ...
  class AdminListUsers {
    ...
    void adminListUsers() throws Exception {
      ...
      mvc.perform(get("/api/auth/admin/users").header("Authorization", "Bearer " + adminToken))
          .andExpect(status().isOk())
    ...
    void managerForbidden() throws Exception {
      mvc.perform(get("/api/auth/admin/users").header("Authorization", "Bearer " + managerToken))
          .andExpect(status().isForbidden());
    }
    ...
    void userForbidden() throws Exception {
      mvc.perform(get("/api/auth/admin/users").header("Authorization", "Bearer " + userToken))
          .andExpect(status().isForbidden());
    }
  }
  ...
  class AdminUpdateRole {
    ...
    void adminUpdatesRole() throws Exception {
      ...
      mvc.perform(
              put("/api/auth/admin/users/10/role")
                  .header("Authorization", "Bearer " + adminToken)
                  .contentType(MediaType.APPLICATION_JSON)
                  .content("{\"role\":\"MANAGER\"}"))
          .andExpect(status().isOk())
  ...
  class AdminToggleDisabled {
    ...
    void adminDisablesUser() throws Exception {
      ...
      mvc.perform(
              put("/api/auth/admin/users/5/disabled")
                  .header("Authorization", "Bearer " + adminToken)
                  .contentType(MediaType.APPLICATION_JSON)
  ...
  class WechatQrInfo {
    ...
    void resetWechatConfig() {
      ...
      org.springframework.test.util.ReflectionTestUtils.setField(
          authController, "wechatAppSecret", "");
    ...
    void mpModeWhenMpConfigured() throws Exception {
      org.springframework.test.util.ReflectionTestUtils.setField(
          authController, "mpAppId", "wx-mp-id");
      org.springframework.test.util.ReflectionTestUtils.setField(
          authController, "mpAppSecret", "wx-mp-secret");
      mvc.perform(get("/api/auth/wechat/qr-info"))
          .andExpect(status().isOk())
    ...
    void oauthModeWhenOAuthConfigured() throws Exception {
      org.springframework.test.util.ReflectionTestUtils.setField(
          authController, "wechatAppId", "wx-oauth-id");
      org.springframework.test.util.ReflectionTestUtils.setField(
          authController, "wechatAppSecret", "wx-oauth-secret");
      mvc.perform(get("/api/auth/wechat/qr-info"))
          .andExpect(status().isOk())
