...
public class SmsService {
  ...
  /**
   * 是否为 mock 模式（未配置华信账号）。 用于 AuthController 决定是否在 /send-code 响应里回传 code 字段 —— 仅 dev/mock
   * 模式回传，方便前端自动回填验证码到输入框； 真服务上线后不再回传，前端逻辑自动退化为「用户去查短信」。
   */
   ...
   * 发送验证码到手机号
   *
   * @return 发送成功时返回生成的验证码（方便开发环境直接返回） 发送失败时返回 null
   */
  public String sendCode(String phone, String code) {
    ...
    if (username == null || username.isBlank()) {
      log.info("[华信SMS Mock] phone={} code={}", phone, code);
      return code;
    }
    ...
    try {
      ...
      log.warn("[华信SMS Error] response={}", response);
      return null;
    } catch (Exception e) {
      log.warn("[华信SMS Exception] cause={}", e.getMessage());
      return null;
    }
  }
