...
ALTER TABLE invest_position_common
    ...
    ADD COLUMN atr_alert_amplitude  DECIMAL(8,3) DEFAULT NULL                   COMMENT 'ATR 振幅倍数(例如 1.500 表示 1.5x ATR)',
    ADD COLUMN atr_alert_enabled    TINYINT(1)   NOT NULL DEFAULT 0            COMMENT '启用 ATR 振幅触发',
    ADD COLUMN stop_loss_pct        DECIMAL(8,2) DEFAULT NULL                  COMMENT '%-based 止损(存负数, -8.00 = -8%)',
