...
/**
 * 纯函数规则引擎：输入 MonitorContext，输出 0..N 个 MonitorSignal。
 * 不做持久化，不做推送 — 调用方负责 dispatch。
 *
 * 评估 5 类规则，按优先级顺序短路评估：
 *  1. 固定买入价 (fixed_buy_enabled=1 + latest <= fixedBuyPrice)
 *  2. 固定卖出价 (fixed_sell_enabled=1 + latest >= fixedSellPrice)
 *  3. ATR 振幅 (atr_alert_enabled=1 + |latest - openToday| >= atrAlertAmplitude * ATR)
 *  4. 止盈 (entryPrice != null + takeProfitPct != null + latest >= target)
 *  5. %-止损 (entryPrice != null + stopLossPct != null + latest <= floor)
 *  6. ATR 移动止损 (useAtr=1 + peakPrice != null + latest <= stopLine)
 */
@Component
    ...
    private void evalAtrAmplitude(MonitorContext ctx, List<MonitorSignal> out) {
        ...
        if (move.compareTo(threshold) >= 0) {
            out.add(MonitorSignal.atrAmplitude(pos, ctx.getStockCode(), ctx.getStockName(),
                    ctx.getLatest(), ctx.getOpenToday(), ctx.getAtr(), pos.getAtrAlertAmplitude()));
        }
    }
