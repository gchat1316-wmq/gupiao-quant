...
class UnifiedStockResearchServiceTest {
    ...
    void buildUnifiedResponseDegradesMissingSources() {
        ...
        StockAnalysisResponse response = service.buildUnifiedResponse(
                ...
                Map.of(
                        ...
                        "quote", Map.of("close", 25.30, "turn", 0.12, "period_high", 28.0, "period_low", 21.0, "period_change_pct", 0.18),
                        "financial_history", List.of(
                                Map.of(
                                        "statDate", "2025-12-31",
                                        "profitability", Map.of("roe_avg", 0.15, "gp_margin", 0.31, "np_margin", 0.12, "eps_ttm", 1.23),
        ...
        assertThat(response.getSourceMetadata()).containsKeys("db", "baostock", "forecast", "webSearch", "aStockData");
        assertThat(((Map<?, ?>) response.getSourceMetadata().get("forecast")).get("available")).isEqualTo(false);
        assertThat(((Map<?, ?>) response.getSourceMetadata().get("webSearch")).get("available")).isEqualTo(false);
    ...
    void buildUnifiedResponseIncludesForecastAndSearchSummary() {
        ...
        assertThat(((Map<?, ?>) response.getSourceMetadata().get("forecast")).get("available")).isEqualTo(true);
