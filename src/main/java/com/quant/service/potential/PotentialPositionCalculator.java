 ...
 *   <li>{@link #effectiveTargetPrice(InvestPositionCommon)}：有效目标价 = pos.targetSellPrice ?? entry ×
 *       (1 + takeProfitPct/100)
...
public class PotentialPositionCalculator {
  ...
  /**
   * 构造一份潜在监控默认持仓记录（未持久化）。单一来源：{@link #getOrCreatePosition(String)} 与 {@code
   * PotentialService.addToPool} 都通过它来消除重复初始化逻辑。
   *
