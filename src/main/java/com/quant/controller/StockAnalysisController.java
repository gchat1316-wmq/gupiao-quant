...
public class StockAnalysisController {
  ...
  public Map<String, Object> status(@PathVariable Long id) {
    ...
    if ("RUNNING".equals(rec.getStatus()) && rec.getStartedAt() != null) {
      long runningMs =
          java.time.Duration.between(rec.getStartedAt(), java.time.LocalDateTime.now()).toMillis();
  ...
  /** 懒生成 + 返回 PDF (一次性 stream 下载) - 首次调用生成, 之后复用 - SUCCESS 状态才允许 */
  @GetMapping(value = "/pdf/{id}")
  public void pdf(@PathVariable Long id, jakarta.servlet.http.HttpServletResponse response)
    ...
    if (!"SUCCESS".equals(rec.getStatus())) {
      ...
      response
          .getWriter()
          .write(
              "{\"ok\":false,\"code\":400,\"message\":\"仅 SUCCESS 状态可导出, 当前: "
                  + rec.getStatus()
                  + "\"}");
      return;
    }
    try {
      ...
      String displayName =
          (rec.getStockName() == null ? rec.getStockCodeRaw() : rec.getStockName())
              + "-"
              + rec.getStockCodeRaw()
              + "-分析报告.pdf";
      String encoded =
      ...
      response
          .getWriter()
          .write(
              "{\"ok\":false,\"code\":500,\"message\":\"PDF 生成失败: "
