 ...
 *
 * 思路：股票池里的"科技风投"类型条目基本都有完整的季度财务数据，但预测字段常年为空。
 * 这里取最近 4 个季度的累计营收趋势，按几何 CAGR 外推 Y0/Y1/Y2 三个预测年份。
 * 历史营收和 Q1 财务指标也顺手从同一张表回填（InvestPoolRefreshService 会调过来）。
 *
 * 单位换算：{@code trade_stock_financial.revenue} 单位是"元"，股票池字段单位是"亿元"，
 * 这里统一除以 1e8 输出，保留两位小数。
 */
@Slf4j
@Service
public class AStockDataForecastProvider implements InvestForecastProvider {
    ...
    /**
     * 用最近 4 个累计营收做几何 CAGR 外推。
     * - 趋势样本不足时回退到"最新同比 * 上一累计营收"
     * - 都没有时返回 null（交给上层不要硬填）
     *
     * 注意：{@code trade_stock_financial.revenue_yoy} 单位是 %（例如 67.17 表示 67.17%），
     * 历史脏数据里偶尔会有 999999.9999 这种异常值（换季/收购），这里要过滤掉。
     */
     ...
     */
    public record FinancialSnapshot(String stockCode,
                                    TradeStockFinancial latest,
                                    List<TradeStockFinancial> history) {
