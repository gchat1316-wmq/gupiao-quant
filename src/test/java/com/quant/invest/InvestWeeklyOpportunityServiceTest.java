 ...
 * 每周机会点 (3×3 卡片) 服务单测。
 *
 * <p>关键不变量： 1. 每次返回的 slot 数量恒为 9（不足 9 个时用空 slot 补齐） 2. slot 顺序按 slotIndex 升序 3. stockName 从
 * invest_stock_pool 联动 — 股票已不在股票池时为 null 4. update() 是「全量替换」语义：先删旧 9 行，再插新 9 行（事务内） 5. update() 拒绝
 * poolType ∉ {tech_ai, innovative_drug, quality} 6. update() 拒绝 slotIndex ∉ [0, 8] 7. update() 拒绝
 * slotIndex 重复 8. level 字段不由 service 计算（前端用 inferValuationRange 自算）
 */
...
class InvestWeeklyOpportunityServiceTest {
  ...
  private final InvestStockPoolRepository stockPoolRepo = mock(InvestStockPoolRepository.class);
  ...
  void getPadsMissingSlots() {
    when(repo.findByPoolTypeOrderBySlotIndexAsc("innovative_drug"))
        .thenReturn(List.of(slot(0, "300760", "回踩到位"), slot(2, "600276", "GLP-1 催化")));
    when(stockPoolRepo.findByPoolTypeOrderByCreatedAtDesc("innovative_drug"))
        .thenReturn(List.of(stock("300760", "迈瑞医疗"), stock("600276", "恒瑞医药")));
  ...
  void getStockNameNullWhenStockNotInPool() {
    when(repo.findByPoolTypeOrderBySlotIndexAsc("quality"))
        .thenReturn(
            List.of(
                slot(0, "000858", "PE 14×"), slot(1, "999999", "已退市") // 不在股票池里
                ));
    when(stockPoolRepo.findByPoolTypeOrderByCreatedAtDesc("quality"))
        .thenReturn(
            List.of(
                stock("000858", "五粮液")
                // 999999 不在列表
  ...
  void updateReplacesAllNineSlots() {
    when(stockPoolRepo.findByPoolTypeOrderByCreatedAtDesc("tech_ai"))
        .thenReturn(List.of(stock("002371", "北方华创"), stock("300750", "宁德时代")));
