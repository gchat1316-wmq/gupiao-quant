...
class WindResearchServiceTest {
    ...
    @Mock
    WindAifinMarketClient windClient;
    ...
    void fetchConsensusParsesAnalyticsDataResponse() throws Exception {
        ...
        var node = buildAnalyticsDataResponse(
    ...
    /**
     * 构造 Wind analytics_data.get_financial_data 风格的 mock 返回。
     * 外层 MCP result: {content:[{text:"<json string>"}]}
     * 内层: {data:{data:[{columns, rows, resolved_question}]}}
     */
