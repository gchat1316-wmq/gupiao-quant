 ...
 * 邮件服务。
 *
 * <p>Dev 环境（未配 SMTP_HOST）时直接打印验证码到日志，便于本地调试。 Prod 环境配置
 * SMTP_HOST/SMTP_PORT/SMTP_USERNAME/SMTP_PASSWORD/SMTP_FROM 后， 通过注入的 WebClient 调用 SMTP 网关。
 */
@Slf4j
@Service
public class EmailService {
  ...
  /**
   * 是否为 mock 模式（未配置 SMTP 主机）。 用于 AuthController 决定是否在 /send-email-code 响应里回传 code 字段 —— 仅 dev/mock
   * 模式回传，方便前端自动回填验证码到输入框； 真服务上线后不再回传，前端逻辑自动退化为「用户去查邮箱」。
   */
  ...
  public String sendCode(String email, String code) {
    ...
    if (smtpHost == null || smtpHost.isBlank()) {
      log.info("[Email Mock] to={} code={}", email, code);
      return code;
    }
  ...
  private String sendViaApi(String email, String code) {
    try {
      ...
      log.warn("[Email Error] cause={}", e.getMessage());
      return null;
    }
  }
