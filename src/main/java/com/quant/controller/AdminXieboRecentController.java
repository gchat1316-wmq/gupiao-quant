...
public class AdminXieboRecentController {
    ...
    public ResponseEntity<Map<String, Object>> create(@RequestBody @Valid AdminRecentStockRequest req) {
        if (watchRepo.existsById(req.getStockCode())) {
            return ResponseEntity.badRequest().body(Map.of(
                    "ok", false,
                    "errorCode", "DUPLICATE",
                    "errorMessage", "股票已存在: " + req.getStockCode()));
        }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public Map<String, Object> update(@PathVariable String stockCode,
                                      @RequestBody @Valid AdminRecentStockRequest req) {
        InvestXieboRecentWatch w = watchRepo.findById(stockCode)
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public Map<String, Object> upsertNote(@PathVariable String stockCode,
                                          @RequestBody AdminNoteUpdateRequest req) {
        // JSoup sanitize: 允许基本标签 + 图片
        String sanitized = req.getNoteHtml() == null ? null
                : Jsoup.clean(req.getNoteHtml(), Safelist.basicWithImages());
        InvestXieboStockNote note = noteRepo.findById(stockCode)
                .orElseGet(() -> {
