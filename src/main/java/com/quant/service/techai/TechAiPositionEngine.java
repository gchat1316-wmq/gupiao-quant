...
/** 持仓策略引擎：根据持仓聚合 + 现价 + ATR 计算下一加仓价、移动止损、浮动盈亏与待办信号。 纯计算，不做任何持久化；止损"只升不降"通过 max(已存止损, 计算止损) 实现。 */
@Component
public class TechAiPositionEngine {
  ...
  public PositionPlan evaluate(PoolView p, BigDecimal price, BigDecimal atr) {
    ...
    boolean hasPosition =
        lots != null && lots.compareTo(BigDecimal.ZERO) > 0 && p.getAvgCost() != null;
    ...
    if (useAtr) {
      computedStop =
          effectivePeak.subtract(mult(p.getAtrTrailMult(), BigDecimal.valueOf(2)).multiply(atr));
    } else {
    ...
    BigDecimal effectiveStop =
        p.getStopPrice() == null ? computedStop : p.getStopPrice().max(computedStop);
    ...
    BigDecimal floatingPnl =
        price
            .subtract(avg)
            .multiply(lots)
            .multiply(BigDecimal.valueOf(SHARES_PER_LOT))
  ...
  /** 策略路线图预演：给定入场价和参数，计算每一档的买入价/止损/均价。 用于 watching 状态（未建仓）时展示"如果现在入场，计划是怎样的"。 */
    ...
    for (int i = 0; i < schedule.size(); i++) {
      ...
      if (i == 0) {
        ...
        if (useAtr) {
          ...
          buyPrice =
              lastBuyPrice.multiply(BigDecimal.ONE.add(step)).setScale(2, RoundingMode.HALF_UP);
        }
      }
