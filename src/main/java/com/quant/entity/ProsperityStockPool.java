 ...
 *
 * <p>与 {@code invest_stock_pool}(龙江投资股票池) 独立：前者面向热点选股流水线 的短线/波段跟踪，后者面向中长期持仓。同一只股票在两个池子里的状态互不影响。
 *
 * <p>同一只股票只入一次，重复"入池"等价于刷新快照：累加 {@link #poolCount}、 更新 {@link #lastAddedAt} / {@link
 * #lastSnapDate}、在 {@link #memo} 末尾追加新推荐理由。
 */
@Getter
@Setter
@Entity
...
public class ProsperityStockPool {
  ...
  /** 所有者 user_id。NULL 表示系统共享（兼容历史数据）。 唯一约束改为 (owner_id, stock_code) 组合。 */
