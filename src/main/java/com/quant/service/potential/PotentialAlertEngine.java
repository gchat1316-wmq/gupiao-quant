 ...
 *
 * <ul>
 *   <li>{@link #monitorQuotes(NotificationProperties.QuoteMonitor, List, Map, Map)}：盘中扫盘（% 阈值告警 +
 *       持仓信号 + MonitorService 融合扫描）
 *   <li>{@link #confirmPositionSignals(NotificationProperties.QuoteMonitor, List)}：收盘确认（按
 *       a-stock-data 实时价定位）
...
public class PotentialAlertEngine {
      ...
      InvestPositionCommon position =
          positionRepository
              .findByStockCodeAndPoolType(
                  item.getStockCode(), PotentialPositionCalculator.POOL_TYPE_POTENTIAL)
              .orElse(null);
      String stockName =
          PotentialPoolSupport.displayStockName(
              item, quoteAggregator.basicFromMap(basics, item.getStockCode()));
  ...
  public int confirmPositionSignals(
    ...
    for (PotentialPool item : pool) {
      ...
      BigDecimal atr =
          positionCalculator.isAtrMode(position)
              ? positionCalculator.atrFor(position, item.getStockCode())
              : null;
  ...
  boolean pushPositionSignal(
    ...
    String stockName =
        PotentialPoolSupport.displayStockName(item, quoteAggregator.basic(item.getStockCode()));
    ...
    String advice =
        switch (signal) {
          ...
          case TechAiPositionEngine.SIGNAL_ADD ->
              String.format(
                  "现价突破加仓位，建议加仓 %s 手。",
                  plan.getNextAddLots() == null
                      ? "-"
                      : PotentialPoolSupport.fmt(plan.getNextAddLots()));
          case TechAiPositionEngine.SIGNAL_TP ->
              String.format(
                  "现价达到目标价，建议减仓 %s%% 止盈。",
                  position == null || position.getTakeProfitPct() == null
                      ? "50"
                      : PotentialPoolSupport.fmt(position.getTakeProfitPct()));
          default -> "";
        };
