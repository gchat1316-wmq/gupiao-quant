 ...
 *
 * <p>历史教训：2026-06-27 feat(auth) 提交加了 Security 链， 但 permitAll 名单只放行了 /api/**，导致 .html / .css / .js
 * 全部 403。 整个前端页面打不开，但进程不崩，restart.sh 自检通过， 容易误判为"服务挂了"而实际是"服务没给前端开门"。
 *
 * <p>回归保险：以下路径必须在不携带 token 的情况下返回非 401/403 状态。 关键是绝对不能 401/403——那才是 Security 拦了。
 *
 * <p>选用 AuthController.class 作为 WebMvcTest 入口是顺手的： 它跟其他 controller 一样存在于 com.quant.controller 包下，
 * WebMvcTest 会自动导入同包的所有 @ControllerAdvice（如有）， 再加上 @Import(SecurityConfig.class, ...) 把 Security
 * 链挂上。 业务 bean 通过 @MockBean 隔离掉，本测试只关心 Security 的 permitAll 决策。
 */
@WebMvcTest(
    controllers = AuthController.class,
    properties = {"app.jwt.secret=test-secret-key-at-least-32-chars-long-for-hs256!"})
...
class SecurityConfigStaticResourceTest {
  ...
  @MockBean private com.quant.service.AuthService authService;
  @MockBean private UserRepository userRepository;
  @MockBean private com.quant.service.wechat.WechatMpService wechatMpService;
  @MockBean private com.quant.service.SmsService smsService;
  @MockBean private com.quant.service.EmailService emailService;
  @MockBean private com.quant.service.wechat.WechatScanSession wechatScanSession;

  /** 状态码既不是 401 也不是 403（即没被 Security 链拒掉） */
  private static final ResultMatcher NOT_SECURITY_BLOCKED =
      result -> {
        int status = result.getResponse().getStatus();
        if (status == 401 || status == 403) {
          throw new AssertionError(
              "Expected NOT 401/403 (Security should not block), but got " + status);
        }
      };
  ...
  void staticAssetDirsArePublic() throws Exception {
    for (String path :
        new String[] {
          "/css/invest.css", "/css/app.css", "/js/invest.js", "/js/app.js", "/lib/chart.js"
        }) {
      mvc.perform(get(path)).andExpect(NOT_SECURITY_BLOCKED);
    }
  }
