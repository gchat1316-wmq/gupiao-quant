...
/** 板块叙事 AI 解读: - 主调 MiniMax,失败回退 SenseNova,最后回退到模板兜底。 - 联网检索摘要由 WebSearchClient 提供(失败时静默忽略)。 */
@Slf4j
@Service
...
public class SectorNarrativeService {
  ...
  private String buildPrompt(ProsperityHotSector s) {
    ...
    if (webSearch.isEnabled()) {
      try {
        List<WebSearchClient.SearchResult> r =
            webSearch.search(s.getSectorName() + " 板块 利好 政策 业绩 近期");
        if (!r.isEmpty()) {
          sb.append("\n联网检索摘要:\n");
  ...
  private String fallback(ProsperityHotSector s) {
    return String.format(
        "【%s】板块综合评分 %s,当日涨幅 %s%%。AI 服务暂未开启或调用失败,请稍后通过手动触发刷新叙事。",
        s.getSectorName(),
