...
CREATE TABLE `prosperity_leader_candidate` (
  ...
  `finance_passed` tinyint(1) DEFAULT NULL COMMENT 'Step3财务硬筛是否通过',
  `finance_reason` varchar(256) DEFAULT NULL COMMENT 'Step3未通过原因',
  `mainline_score` decimal(8,2) DEFAULT NULL COMMENT 'Step4主线评分',
  `mainline_passed` tinyint(1) DEFAULT NULL COMMENT 'Step4主线判定是否通过',
  `mainline_reason` varchar(256) DEFAULT NULL COMMENT 'Step4主线未通过原因',
  `revenue_yoy_min_4q` decimal(10,4) DEFAULT NULL COMMENT '近4季营收同比最小值(%)',
  `deducted_netprofit_yoy_min_4q` decimal(10,4) DEFAULT NULL COMMENT '近4季扣非同比最小值(%)',
  `gross_margin_avg_4q` decimal(10,4) DEFAULT NULL COMMENT '近4季毛利率均值(%)',
  `debt_ratio_latest` decimal(10,4) DEFAULT NULL COMMENT '最新资产负债率(%)',
  `operating_cashflow_sum_4q` decimal(20,2) DEFAULT NULL COMMENT '近4季经营现金流合计(元)',
  `roe_latest` decimal(10,4) DEFAULT NULL COMMENT '最新 ROE(%)',
  `final_stage` varchar(20) DEFAULT NULL COMMENT '最终到达阶段: leader_filter/finance_filter/mainline_filter/passed',
  `created_at` datetime DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
