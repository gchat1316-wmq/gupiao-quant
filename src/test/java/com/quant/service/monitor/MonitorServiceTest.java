...
class MonitorServiceTest {
    ...
    void setUp() {
        ...
        service = new MonitorService(posRepo, alertRepo, dailyRepo, quoteService,
                notificationService, ruleEngine, atrCalculator, props);
    }
    ...
    private AStockDataQuoteService.QuoteSnapshot quote(String code) {
        return new AStockDataQuoteService.QuoteSnapshot(
                code, new BigDecimal("1480.00"), new BigDecimal("1500.00"),
    ...
    void scanInvokesQuoteFetchAndRuleEngineForEachWatchedStock() {
        ...
        when(ruleEngine.evaluate(any())).thenReturn(List.of(
                MonitorSignal.fixedPriceBuy(p, "600519.SH", "贵州茅台",
                        new BigDecimal("1480.00"), new BigDecimal("1500.00"))));
