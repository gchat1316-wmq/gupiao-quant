 ...
 *
 * <ul>
 *   <li>当前市值 &lt; Y1×10 → {@code 低估}
 *   <li>Y1×10 ≤ 当前市值 ≤ Y2×10 → {@code 合理}
 *   <li>当前市值 &gt; Y2×10 → {@code 泡沫}（需 2-3 年营收增长才能支撑）
 * </ul>
 *
 ...
 *
 * <p>2026-07-01 改：去掉"净利率 ≥ 25%"硬门槛。截图里的 6.29%（长盈通）/14.51%（东威） 也已纳入 10×PS 估值；commentary
 * 里仍标注净利率水平供用户参考。
 */
@Slf4j
@Service
...
public class Ps10ValuationService {
  ...
  public record Ps10Result(
    ...
    public static Ps10Result inapplicable(String verdict, String commentary, String method) {
      return new Ps10Result(
          false, verdict, commentary, method, null, null, null, null, null, null, null, null);
    }

    public static Ps10Result inapplicableWithMargin(
        String verdict, String commentary, BigDecimal netMargin, BigDecimal marketCap) {
      return new Ps10Result(
          false,
          verdict,
          commentary,
    ...
    public static Ps10Result applicable(
        String verdict,
        String commentary,
        BigDecimal ttm,
        BigDecimal y1,
    ...
    public BigDecimal deviationPct() {
      return deviation().pct();
    }

    public String deviationRef() {
    ...
    private Deviation deviation() {
      ...
      BigDecimal pct =
          diff.divide(ref, 4, RoundingMode.HALF_UP)
   ...
   *
   * <p>用于"科技风投 · 10 倍 PS 法"计算器：用户输入市值 + Y0/Y1/Y2 预测营收， 后端直接算 verdict + commentary + 各年 PS
   * 倍数，避免前端重复实现估值逻辑。
   *
   ...
   * @since 2026-07-01
   */
  public Ps10Result evaluateFromInputs(
      BigDecimal marketCap,
    ...
    if (marketCap == null || marketCap.signum() <= 0) {
      ...
      double discount =
          fairCapY1.subtract(marketCap).divide(fairCapY1, 4, RoundingMode.HALF_UP).doubleValue()
              * 100;
      verdict = "低估";
      commentary =
          String.format(
      ...
      double premium =
          marketCap.subtract(fairCapY2).divide(fairCapY2, 4, RoundingMode.HALF_UP).doubleValue()
              * 100;
      verdict = "泡沫";
      commentary =
          String.format(
      ...
      double pct =
          marketCap
              .subtract(refCap)
              .divide(refCap, 6, RoundingMode.HALF_UP)
              .multiply(BigDecimal.valueOf(100))
              .doubleValue();
      verdict = "合理";
      commentary =
    ...
    if (netMarginPct == null) {
      ...
      commentary += String.format(Locale.ROOT, "（注：净利率 %.2f%% 偏低，10×PS 法适用性有限，仅作参考）", netMarginPct);
    }
  ...
  public Ps10Result evaluate(String stockCode, BigDecimal marketCapOverride) {
    ...
    return evaluateFromMarketCap(
        marketCap, snapshot != null ? snapshot.latestPrice() : null, stockCode, null);
  }

  /** 核心评估方法（无财务数据时调用）。 */
  public Ps10Result evaluateFromMarketCap(
      BigDecimal marketCap, BigDecimal price, String stockCode) {
  ...
  public Ps10Result evaluateFromMarketCap(
    ...
    if (marketCap == null) {
      ...
      double premium =
          marketCap.subtract(fairCapY1).divide(fairCapY1, 4, RoundingMode.HALF_UP).doubleValue()
              * 100;
      verdict = "合理";
      commentary =
          String.format(
