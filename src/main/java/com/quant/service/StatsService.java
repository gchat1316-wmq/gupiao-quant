...
public class StatsService {
  ...
  public void recordPageView(Long userId, String pagePath, String sessionId, String userAgent) {
    try {
      ...
      if (sessionId != null && !sessionId.isBlank()) {
        var prevOpt =
            pageViewStatRepo.findTopBySessionIdAndVisitDateOrderByVisitTimeDesc(sessionId, today);
  ...
  private void updateUserDailyStat(
      Long userId, LocalDate date, int durationSeconds, String pagePath) {
    UserDailyStat stat =
        userDailyStatRepo
  ...
  public StatsPageDTO getStatsPage(LocalDate date) {
    ...
    int uv =
        uvRows.isEmpty()
            ? 0
            : (uvRows.get(0)[2] == null ? 0 : ((Number) uvRows.get(0)[2]).intValue());
        ...
        topUsers.stream()
            ...
            .map(
                us -> {
                  ...
                  usd.setTotalDurationSec(
                      us.getTotalDurationSeconds() == null ? 0 : us.getTotalDurationSeconds());
                  usd.setAvgDuration(
                      formatDuration(usd.getTotalDurationSec(), usd.getPageViewCount()));
                  userRepository
