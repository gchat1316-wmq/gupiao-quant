 ...
 * 仓位管理系统 — 三大数学公式实现:
 *
 * 1) 风险回报比 R:R = (entry - stopLoss) : (target - entry)
 *    盈亏平衡胜率 = 1 / (1 + reward/risk)
 *
 * 2) 期望值 EV = winRate * avgWin - (1 - winRate) * avgLoss
 *    其中 avgWin = target - entry, avgLoss = entry - stopLoss,均按每股计
 *
 * 3) 仓位管理 Position = accountCapital * riskPercent / (entry - stopLoss)
 *    A 股最小交易单位 100 股,实际可买 = floor(shares / 100) * 100
 *
 * 参考 Mark Minervini / Jesse Livermore / Michael Marcus 的纪律:
 *   - 风报比 ≥ 1:3 才进场
 *   - 单笔风险 ≤ 账户 1-2%
 *   - 仓位一致,不因把握度而放大
 */
...
public class PositionManagementService {
    ...
    public PositionAdviceDTO advise(PositionAdviceRequest req) {
        ...
        BigDecimal riskPct = req.getRiskPercent() == null
                ? new BigDecimal("0.01")
                : req.getRiskPercent();
        BigDecimal winRate = req.getWinRate() == null
                ? new BigDecimal("0.40")
                : req.getWinRate();
        ...
        BigDecimal lots = sharesRaw.divide(ONE_HUNDRED, 0, RoundingMode.DOWN).setScale(0, RoundingMode.DOWN);
        ...
        PositionAdviceDTO.Sizing sizing = PositionAdviceDTO.Sizing.builder()
        ...
        BigDecimal breakEven = BigDecimal.ONE.divide(
                BigDecimal.ONE.add(ratioValue), 4, RoundingMode.HALF_UP);
        ...
        PositionAdviceDTO.RiskReward riskReward = PositionAdviceDTO.RiskReward.builder()
        ...
        BigDecimal evPerShare = winRate.multiply(rewardPerShare)
                .subtract(BigDecimal.ONE.subtract(winRate).multiply(riskPerShare));
        BigDecimal expectedValuePerTrade = evPerShare.multiply(actualShares)
                .setScale(2, RoundingMode.HALF_UP);
        BigDecimal expectedValue100 = expectedValuePerTrade.multiply(ONE_HUNDRED)
                .setScale(2, RoundingMode.HALF_UP);
        String evVerdict;
        ...
        PositionAdviceDTO.Expectation expectation = PositionAdviceDTO.Expectation.builder()
                .winRate(winRate)
        ...
        PositionAdviceDTO.InputSummary inputSummary = PositionAdviceDTO.InputSummary.builder()
    ...
    private List<PositionAdviceDTO.DrawdownRow> buildDrawdownTable(BigDecimal riskPct) {
        ...
        for (int n = 1; n <= DEFAULT_DRAWDOWN_ROWS; n++) {
            ...
            BigDecimal recover = BigDecimal.ONE.divide(remaining, 4, RoundingMode.HALF_UP)
                    .subtract(BigDecimal.ONE).setScale(4, RoundingMode.HALF_UP);
            rows.add(PositionAdviceDTO.DrawdownRow.builder()
                    .consecutiveLosses(n)
    ...
    private String composeVerdict(String rrVerdict, String evVerdict,
                                   BigDecimal ratioValue, BigDecimal breakEven, BigDecimal winRate) {
    ...
    private String composeSummary(BigDecimal account, BigDecimal riskPct,
                                   BigDecimal ratioValue, BigDecimal breakEven, BigDecimal winRate) {
    ...
    private List<String> composePrinciples(String rrVerdict, String evVerdict,
                                           BigDecimal lots, BigDecimal positionPct) {
        ...
        if (lots.compareTo(BigDecimal.ZERO) == 0) {
            ...
            ps.add(String.format("实际建议买入 %s 手(共 %s 股),占账户 %s%%。",
                    stripTrailing(lots),
