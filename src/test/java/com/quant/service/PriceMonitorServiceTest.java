...
class PriceMonitorServiceTest {
    ...
    void triggersBuyAlertAndDispatches() {
        ...
        AStockDataQuoteService.QuoteSnapshot snap = new AStockDataQuoteService.QuoteSnapshot(
                "600519", new BigDecimal("1750"), null, null, null, null);
        when(aStockDataQuoteService.fetchQuotes(anyList())).thenReturn(java.util.Map.of("600519", snap));

        when(notificationDispatcher.dispatchPriceAlert(eq("600519"), eq(PriceMonitorService.TYPE_BUY), anyString(), anyString()))
                .thenReturn(new NotificationDispatcher.DispatchResult(3, 3));

        service.monitorPrices();

        ArgumentCaptor<String> typeCaptor = ArgumentCaptor.forClass(String.class);
        verify(notificationDispatcher).dispatchPriceAlert(eq("600519"), typeCaptor.capture(), anyString(), anyString());
    ...
    void resetsAlertStateWhenPriceNormal() {
        ...
        verify(notificationDispatcher, never()).dispatchPriceAlert(anyString(), anyString(), anyString(), anyString());
    }
    ...
    void triggersSellAlertAndDispatches() {
        ...
        when(aStockDataQuoteService.fetchQuotes(anyList())).thenReturn(java.util.Map.of("002371", snap));

        when(notificationDispatcher.dispatchPriceAlert(eq("002371"), eq(PriceMonitorService.TYPE_SELL), anyString(), anyString()))
                .thenReturn(new NotificationDispatcher.DispatchResult(2, 2));

        service.monitorPrices();

        verify(notificationDispatcher).dispatchPriceAlert(eq("002371"), eq(PriceMonitorService.TYPE_SELL), anyString(), anyString());
