 ...
 *
 * 覆盖 F5: NotificationDispatcher.dispatchPriceAlert
 *  - SMS 通知按用户偏好 fanout 到 SmsService
 *  - 微信通知 fanout 到 ServerChan
 *  - disabled / 空目标用户不会收到
 *  - 重度用户发了 fanout 后落入 user_notification_log
 */
...
class HeavyUserSmsNotificationTest {
    ...
    @BeforeEach
    void setUp() {
        dispatcher = new NotificationDispatcher(userRepository, logRepository, smsService, notificationService);
    }
    ...
    void heavyUsersGetSmsPerUser() {
        when(userRepository.findActiveSmsTargets()).thenReturn(List.of(
            heavyUser(1L, "13800000001", false),
            heavyUser(2L, "13800000002", false)
        ));
        ...
        var result = dispatcher.dispatchPriceAlert("600519.SH", "PRICE_BUY_ALERT",
            "买入", "现价 1234.56");
    ...
    void heavyUserWithoutPhoneIsFilteredByRepo() {
        when(userRepository.findActiveSmsTargets()).thenReturn(List.of(
            heavyUser(3L, "13800000003", false)
        ));
        when(userRepository.findActiveByWechat(true)).thenReturn(List.of());
        when(smsService.sendAlarm(eq("13800000003"), anyString())).thenReturn(true);

        dispatcher.dispatchPriceAlert("002371", "PRICE_SELL_ALERT",
            "卖出", "现价 234.56");
    ...
    void wechatUsersGetServerChan() {
        when(userRepository.findActiveSmsTargets()).thenReturn(List.of());
        when(userRepository.findActiveByWechat(true)).thenReturn(List.of(
            wechatUser(10L), wechatUser(11L)
        ));
        when(notificationService.sendServerChan(anyString(), anyString())).thenReturn(true);

        var result = dispatcher.dispatchPriceAlert("002371", "PRICE_BUY_ALERT",
            "买入", "现价");
    ...
    void smsFailurePersistsFailedLog() {
        when(userRepository.findActiveSmsTargets()).thenReturn(List.of(
            heavyUser(20L, "13800000020", false)
        ));
        when(userRepository.findActiveByWechat(true)).thenReturn(List.of());
