 ...
 *
 * <p>RED phase: 先写期望行为 → 跑不过 → 修代码 覆盖： 1. promote(ownerId) 个人池：不同 owner 的同一股票互不影响 2. promote(NULL)
 * 系统池：共享数据 3. list(ownerId) 返回个人池 + 系统池 4. 重复入池：累加 poolCount
 */
...
class ProsperityPoolServiceTest {
  ...
  class PromoteWithOwnerId {
    ...
    void newStockForOwner1() {
      ...
      when(pickRepo.findBySnapDateAndStockCode(SNAP, "000001.SZ"))
          .thenReturn(Optional.of(makePick("000001.SZ", "平安银行")));
      ...
      when(poolRepo.save(any(ProsperityStockPool.class)))
          .thenAnswer(
              inv -> {
                ProsperityStockPool p = inv.getArgument(0);
                p.setId(10);
                return p;
              });
      ...
      ArgumentCaptor<ProsperityStockPool> captor =
          ArgumentCaptor.forClass(ProsperityStockPool.class);
    ...
    void sameStockDifferentOwners() {
      ...
      when(pickRepo.findBySnapDateAndStockCode(SNAP, "000001.SZ"))
          .thenReturn(Optional.of(makePick("000001.SZ", "平安银行")));
      when(poolRepo.findByOwnerIdAndStockCode(1L, "000001.SZ"))
          .thenReturn(Optional.of(existingOwner1));
      when(poolRepo.findByOwnerIdAndStockCode(2L, "000001.SZ")).thenReturn(Optional.empty());
      when(poolRepo.save(any(ProsperityStockPool.class))).thenAnswer(inv -> inv.getArgument(0));
  ...
  class ListWithOwnerId {
    ...
    void listReturnsPersonalPlusSystem() {
      ...
      personal.setId(1);
      personal.setOwnerId(1L);
      personal.setStockCode("000001.SZ");
      personal.setLastAddedAt(LocalDateTime.now());

      ProsperityStockPool system = new ProsperityStockPool();
      system.setId(2);
      system.setOwnerId(null);
      system.setStockCode("000002.SZ");
