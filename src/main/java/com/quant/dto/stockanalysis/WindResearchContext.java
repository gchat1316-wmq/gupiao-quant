...
/**
 * Wind 研报上下文：在 StockAnalysisService 主流程里被拉一次（24h 缓存）， 同时塞进 prompt（强制 AI 引用一致预期） 和 response
 * 字段（前端渲染）。
 *
 * <p>字段命名严格遵守 wind-mcp-skill 返回结构： - analytics_data.get_financial_data 的 rows[].columns -
 * financial_docs.get_financial_news 的 items[].title/content/date/relevance
 *
 * <p>失败/无 Key 时 available=false，prompt 注入 "（Wind 研报：未启用）"，主报告不受影响。
 */
@Data
@Builder
@NoArgsConstructor
