 ...
 *
 * 端点：
 *  GET    /api/monitor/pool[?poolType=tech_ai|potential|stock]   列出某池全部监控项
 *  POST   /api/monitor/pool                                      添加监控
 *  PATCH  /api/monitor/pool/{code}/{poolType}/field              部分更新字段
 *  DELETE /api/monitor/pool/{code}/{poolType}                    删除监控
 *  POST   /api/monitor/run[?poolType=...]                        手动触发扫描
 *  GET    /api/monitor/health                                    健康检查
 *
 * 兼容：原本 /api/tech-ai/* 和 /api/potential/* 仍然可用，但数据来自本服务。
 */
@Slf4j
@RestController
...
public class MonitorController {
    ...
    public List<MonitorPoolItemDTO> pool(@RequestParam(required = false) String poolType) {
        ...
        for (InvestPositionCommon p : rows) {
            ...
            out.add(MonitorPoolItemDTO.from(p,
                    nameMap.getOrDefault(p.getStockCode(), p.getStockCode()),
                    latest, dailyChange));
        }
        return out;
    }
    ...
    public Map<String, Object> add(@RequestBody MonitorAddRequest req) {
        ...
        InvestPositionCommon p = posRepo.findByStockCodeAndPoolType(req.getStockCode(), req.getPoolType())
                .orElseGet(() -> {
    ...
    public Map<String, Object> updateField(@PathVariable String stockCode,
        ...
        InvestPositionCommon p = posRepo.findByStockCodeAndPoolType(stockCode, poolType)
                .orElseThrow(() -> new IllegalArgumentException("未找到股票: " + stockCode + " in " + poolType));
    ...
    private void applyField(InvestPositionCommon p, String field, Object value) {
        switch (field == null ? "" : field) {
            ...
            case "serverchanTemplate"  -> p.setServerchanTemplate(value == null ? "standard" : value.toString());
