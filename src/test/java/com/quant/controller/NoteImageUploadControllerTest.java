...
class NoteImageUploadControllerTest {
            ...
            jsonPath("$.url")
                .value(
                    org.hamcrest.Matchers.matchesPattern(
                        "/uploads/notes/\\d{6}/[a-f0-9]{32}\\.png")));
  }

  @Test
  void upload_tooLarge_returns400() throws Exception {
    byte[] data = new byte[6 * 1024 * 1024]; // 6MB
    MockMultipartFile file = new MockMultipartFile("file", "big.png", "image/png", data);
  ...
  void upload_wrongType_returns400() throws Exception {
    ...
    MockMultipartFile file = new MockMultipartFile("file", "test.txt", "text/plain", data);
