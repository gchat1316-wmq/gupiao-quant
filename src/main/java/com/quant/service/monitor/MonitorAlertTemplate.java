...
/**
 * 把 MonitorSignal 渲染成 Server酱 Markdown。支持 3 种模板。 - standard : 标题 + 单段详尽 Markdown（默认） - compact :
 * 单行紧凑摘要，便于在告警风暴中快速扫读 - verbose : 标准 + 元数据块，便于存档回溯
 */
@Component
public class MonitorAlertTemplate {
  ...
  private static String compact(MonitorSignal sig) {
    return String.format(
        "%s · %s · 现价 %s",
        sig.getTitle() == null ? "" : sig.getTitle(), sig.getStockCode(), sig.getTriggerPrice());
  }
