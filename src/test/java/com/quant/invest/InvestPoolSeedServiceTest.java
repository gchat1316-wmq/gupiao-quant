...
class InvestPoolSeedServiceTest {
  ...
  void replaceTechAiKeepsScreenshotOrder() {
    ...
    verify(poolRepo)
        .deleteByPoolTypeOrUpperStockCodeIn(
            org.mockito.Mockito.eq("tech_ai"),
    ...
    assertThat(rows.subList(0, 17))
        .extracting(InvestStockPool::getStockName)
        .containsExactly(
            "埃科光电", "裕太微", "路维光电", "鼎通科技", "仕佳光子", "华丰科技", "思瑞浦", "睿创微纳", "金海通", "日联科技", "奕瑞科技",
            "凯格精机", "东威科技", "芯碁微装", "杰普特", "长川科技", "拓荆科技");
    assertThat(rows.subList(17, 32))
        .extracting(InvestStockPool::getStockName)
        .containsExactly(
            "长盈通", "思泰克", "奥来德", "钧崴电子", "伟测科技", "快克智能", "晶方科技", "安集科技", "华海清科", "菲利华", "三环集团",
            "峰岹科技", "蓝特光学", "星宸科技", "广合科技");
