...
class InvestServicePoolTest {
  ...
  void listPoolUsesPoolStockNameWhenBasicMissing() {
    ...
    when(stockBasicRepo.findByStockCodeIn(
            org.mockito.ArgumentMatchers.argThat(codes -> codes.contains("688296"))))
        .thenReturn(Collections.emptyList());
    when(financialRepo.findLatestByStockCodes(List.of("688296")))
        .thenReturn(Collections.emptyList());
    when(quoteService.fetchQuotes(List.of("688296"))).thenReturn(Collections.emptyMap());
    when(quoteService.fetchYearStartCloses(
            org.mockito.ArgumentMatchers.eq(List.of("688296")), org.mockito.ArgumentMatchers.any()))
  ...
  void listPoolMatchesBasicCodeCaseInsensitive() {
    ...
    when(stockBasicRepo.findByStockCodeIn(
            org.mockito.ArgumentMatchers.argThat(
                codes -> codes.contains("688525.sh") && codes.contains("688525.SH"))))
        .thenReturn(List.of(basic));
    when(financialRepo.findLatestByStockCodes(List.of("688525.sh")))
        .thenReturn(Collections.emptyList());
    when(quoteService.fetchQuotes(List.of("688525.sh"))).thenReturn(Collections.emptyMap());
    when(quoteService.fetchYearStartCloses(
            org.mockito.ArgumentMatchers.eq(List.of("688525.sh")),
  ...
  void listPoolOrdersTechAiByDisplayOrder() {
    ...
    when(financialRepo.findLatestByStockCodes(List.of("688515.SH", "600519.SH", "688610.SH")))
        .thenReturn(Collections.emptyList());
    when(quoteService.fetchQuotes(List.of("688515.SH", "600519.SH", "688610.SH")))
        .thenReturn(Collections.emptyMap());
    when(quoteService.fetchYearStartCloses(
            eq(List.of("688515.SH", "600519.SH", "688610.SH")), any()))
        .thenReturn(Collections.emptyMap());

    List<PoolItemDTO> result = service.listPool();

    assertThat(result).extracting(PoolItemDTO::getStockName).containsExactly("埃科光电", "裕太微", "贵州茅台");
  }
  ...
  void listPoolPrefersDerivedMarketCapBeforePersistedSnapshot() {
    ...
    when(financialRepo.findLatestByStockCodes(List.of("688610.SH")))
        .thenReturn(Collections.emptyList());
    when(quoteService.fetchQuotes(List.of("688610.SH")))
        .thenReturn(java.util.Map.of("688610.SH", quote));
  ...
  private InvestStockPool pool(
      Integer id, String code, String name, String poolType, Integer displayOrder) {
    InvestStockPool pool = new InvestStockPool();
    pool.setId(id);
  ...
  class UpdatePoolAlertSync {
    ...
    /**
     * 弹窗里勾选"希望买入价 + 消息监控"→ 后端把 target_buy_price 写入 pool， 并把 fixed_buy_enabled=1 +
     * fixed_buy_price=target_buy_price 写入 InvestPositionCommon。 MonitorService 扫描到后会发 server 酱。
     */
    @Test
    ...
    void alertBuyEnabledSyncsToFixedBuy() {
      ...
      when(positionRepo.findByStockCodeAndPoolType("688525.SH", "invest"))
      ...
      ArgumentCaptor<InvestPositionCommon> posCaptor =
          ArgumentCaptor.forClass(InvestPositionCommon.class);
