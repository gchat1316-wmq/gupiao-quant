...
public class XieboRecentAlertJob {
  ...
  private boolean scanOne(UserStockSubscription s, AStockDataQuoteService.QuoteSnapshot q) {
    ...
    boolean any = false;
    if (tryFire(s, cur, s.getPriceBuy(), s.getAlertBuyTriggeredAt(), "xiebo_recent_buy", "买入")) {
    ...
    if (tryFire(
        ...
        cur,
        s.getPriceStopLoss(),
        s.getAlertStopLossTriggeredAt(),
        "xiebo_recent_stop_loss",
        "止损")) {
    ...
    if (tryFire(
        s,
        cur,
        s.getPriceAddPosition(),
        s.getAlertAddPositionTriggeredAt(),
        "xiebo_recent_add",
        "加仓")) {
    ...
    if (tryFire(
        s,
        cur,
        s.getPriceReducePosition(),
        s.getAlertReducePositionTriggeredAt(),
        "xiebo_recent_reduce",
        "减仓")) {
    ...
    if (tryFire(
        s,
        cur,
        s.getPriceClearPosition(),
        s.getAlertClearPositionTriggeredAt(),
        "xiebo_recent_clear",
        "清仓")) {
  ...
  private boolean tryFire(
    ...
    try {
      boolean sent =
          notificationService.sendServerChan(alert.getTitle(), alert.getContent(), sendKey);
