...
CREATE TABLE IF NOT EXISTS invest_weekly_opportunity_slot (
    ...
    image_url       VARCHAR(500) DEFAULT NULL COMMENT 'slot 参考截图 URL（admin 上传）',
...
-- 老库升级：补 user_stock_name / image_url 列
-- ALTER TABLE invest_weekly_opportunity_slot ADD COLUMN user_stock_name VARCHAR(100) DEFAULT NULL COMMENT '用户手工填的股票名（代码不在池中时兜底）';
-- ALTER TABLE invest_weekly_opportunity_slot ADD COLUMN image_url VARCHAR(500) DEFAULT NULL COMMENT 'slot 参考截图 URL（admin 上传）';
