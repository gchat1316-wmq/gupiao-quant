...
class XieboInvestControllerTest {
  ...
  void watchlistEndpointReturnsRows() throws Exception {
    XieboWatchlistItemDTO item =
        XieboWatchlistItemDTO.builder()
            .stockCode("600519.SH")
            .stockName("贵州茅台")
    ...
    when(service.getSectorPe("北方华创"))
        .thenReturn(
            Map.of(
                ...
                new BigDecimal("48.50"),
                "stocks",
                List.of(Map.of("stockCode", "002371.SZ"))));
  ...
  void watchlistMutationsDelegateToService() throws Exception {
    XieboWatchlistItemDTO item =
        XieboWatchlistItemDTO.builder().stockCode("002371.SZ").stockName("北方华创").build();
    when(service.addWatchlist("北方华创")).thenReturn(List.of(item));

    mvc.perform(
            post("/api/xiebo-invest/watchlist")
  ...
  void analysisEndpointsCreateListAndDetail() throws Exception {
    XieboAnalysisDetailDTO detail =
        XieboAnalysisDetailDTO.builder()
            .id(1L)
            .stockCode("002371.SZ")
            .stockName("北方华创")
            .status("completed")
            ...
            .build();
    XieboAnalysisListItemDTO listItem =
        XieboAnalysisListItemDTO.builder()
            .id(1L)
            .stockCode("002371.SZ")
            .stockName("北方华创")
            .status("completed")
    ...
    mvc.perform(
            post("/api/xiebo-invest/analysis")
                .contentType("application/json")
                .content("{\"keyword\":\"北方华创\"}"))
        .andExpect(status().isOk())
