...
class InvestControllerSopTest {
    ...
    private SopCheckupDTO.MetricCheck metric(String label, String verdict, double latest, String tip) {
        return SopCheckupDTO.MetricCheck.builder()
                .label(label).unit("%")
                .series(List.of(
                        SopCheckupDTO.QuarterPoint.builder().quarter("25Q4").value(BigDecimal.valueOf(latest)).build()
                ))
    ...
    void ct01_validStockCode_returns200WithMatchedTrue() throws Exception {
        SopCheckupDTO dto = SopCheckupDTO.builder()
                .matched(true)
                .stockCode("600519")
                .stockName("贵州茅台")
    ...
    void ct02_nonExistentStock_returns200WithMatchedFalse() throws Exception {
        SopCheckupDTO dto = SopCheckupDTO.builder()
                .matched(false)
                .message("未找到股票：北方华创（请输入6位代码或完整名称）")
                .build();
    ...
    void ct03_missingKeyword_returns400() throws Exception {
        mvc.perform(get("/api/invest/sop/checkup"))
    ...
    void ct05_seriesStructureIsCorrect() throws Exception {
        SopCheckupDTO.MetricCheck m = SopCheckupDTO.MetricCheck.builder()
                .label("毛利率").unit("%")
                .series(List.of(
                        SopCheckupDTO.QuarterPoint.builder().quarter("25Q3").value(BigDecimal.valueOf(45.0)).build(),
                        SopCheckupDTO.QuarterPoint.builder().quarter("25Q4").value(BigDecimal.valueOf(43.0)).build()
                ))
                .latest(BigDecimal.valueOf(43.0))
                .verdict("warn").tip("下滑")
                .build();
        SopCheckupDTO dto = SopCheckupDTO.builder()
                .matched(true).stockCode("000002").stockName("系列测试股")
                .grossMargin(m)
                .revenueYoy(metric("营收同比", "pass", 25.0, "ok"))
                .profitYoy(metric("扣非净利润同比", "pass", 28.0, "ok"))
                .overallVerdict("warn").overallSummary("summary")
                .build();
