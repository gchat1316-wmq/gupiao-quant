...
class TechAiAlertRuleEngineTest {
  ...
  void triggersPositiveThresholds() {
    ...
    assertThat(alerts)
        .extracting(TechAiAlertCandidate::dedupeKey)
        .contains(
  ...
  void usesCustomThresholds() {
    TechAiAlertThresholds thresholds =
        TechAiAlertThresholds.builder()
    ...
    List<TechAiAlertCandidate> alerts =
        new TechAiAlertRuleEngine().evaluate(context(new BigDecimal("120")), thresholds);
