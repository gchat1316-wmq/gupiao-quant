...
public class QuoteService {
    ...
    public QuotePageDTO search(String kw, int page, int pageSize) {
        Page<InvestQuote> result = quoteRepo.search(
                (kw == null || kw.isBlank()) ? "" : kw.trim(),
    ...
    public Long importToStudy(Long id) {
        InvestQuote quote = quoteRepo.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("金句不存在: " + id));
        ...
        String title = quote.getContent().length() > 30
        ...
        node.setDefinition(quote.getContent() +
    ...
    private void generateQuiz(StudyKnowledgeNode node, String quoteContent) throws Exception {
        String systemPrompt = "你是一位投资学习助手，请根据投资金句生成一道单选题来检验理解。" +
