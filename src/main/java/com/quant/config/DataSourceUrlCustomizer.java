...
/**
 * Rewrites spring.datasource.url to enable SSL when DB_USE_SSL=true. Idempotent — does nothing in
 * local/dev/test where DB_USE_SSL unset.
 *
 * <p>MySQL server MUST have SSL enabled for useSSL=true to connect. See
 * docs/superpowers/plans/2026-07-15-secret-inventory.md.
 */
public class DataSourceUrlCustomizer implements EnvironmentPostProcessor {
  ...
  public void postProcessEnvironment(ConfigurableEnvironment env, SpringApplication app) {
    ...
    if (rewritten.equals(currentUrl)) {
      System.out.println(
          "[DataSourceUrlCustomizer] DB_USE_SSL=true but URL had no recognizable SSL param; passing through unchanged");
    } else {
      System.out.println(
          "[DataSourceUrlCustomizer] DB_USE_SSL=true → rewrote spring.datasource.url");
