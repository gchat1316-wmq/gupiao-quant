...
public class UnifiedStockResearchService {
  ...
  public StockAnalysisResponse buildUnifiedResponse(
    ...
    Map<String, Object> financialSummary =
        buildFinancialSummary(asList(rawData.get("financial_history")));
    ...
    Map<String, Object> sourceMetadata =
        buildSourceMetadata(
            rawData, dbFinancials, forecastSummary, externalExpectation, windResearch);
  ...
  private String buildFiveDimensionReportHtml(StockAnalysisResponse r, TradeStockBasic basic) {
    ...
    sb.append("<!DOCTYPE html><html lang='zh-CN'><head><meta charset='utf-8'><title>")
        ...
        .append(" 五维模型 · 市值阶梯分析</title>")
        .append("<style>")
        .append(fiveDimCss())
        .append("</style></head><body>");

    Map<String, Object> analysis =
        r.getAnalysis() == null ? Collections.emptyMap() : r.getAnalysis();
    ...
    sb.append("<h1>")
        .append(esc(r.getName()))
        .append(" (")
        .append(esc(r.getCode()))
        .append(") 五维模型 · 市值阶梯分析</h1>");
    ...
    sb.append("<h2>总览</h2><div class='meta-grid'>")
        ...
        .append(
            meta(
                "现价",
                r.getCurrentPrice() == null ? "-" : String.format("%.2f 元", r.getCurrentPrice())))
    ...
    Map<String, Object> sources =
        r.getSourceMetadata() == null ? Collections.emptyMap() : r.getSourceMetadata();
    ...
    sb.append("<h2>① 稀缺卡位 <span class='rating-tag'>")
        .append(esc(value(scarce, "rating")))
        .append("</span></h2>")
        .append(dimRatingNote(value(scarce, "ratingLogic")));
    sb.append("<h3>全球技术稀缺性</h3>")
        .append(
            cards(
                List.of(
    ...
    sb.append("<h3>双赛道卡位</h3>")
        .append(
            cards(
                List.of(
                    card("主业", joinLines(List.of("客户/份额/认证周期: " + value(scarceDual, "主业")))),
                    card("第二曲线", joinLines(List.of("海内外客户矩阵: " + value(scarceDual, "第二曲线")))),
    ...
    sb.append("<h2>② 成长动力 <span class='rating-tag'>")
        .append(esc(value(growth, "rating")))
        .append("</span></h2>")
        .append(dimRatingNote(value(growth, "ratingLogic")));
    sb.append("<h3>第一曲线（稳态保底增长）: ")
        .append(esc(value(firstCurve, "业务名")))
        .append("</h3>")
        .append(
            cards(
                List.of(
    ...
    sb.append("<h3>第二曲线（核心高弹性增长引擎）: ")
        .append(esc(value(secondCurve, "业务名")))
        .append("</h3>")
        .append(
            cards(
                List.of(
    ...
    sb.append("<h2>③ 业绩兑现度 <span class='rating-tag'>")
        .append(esc(value(deliver, "rating")))
        .append("</span></h2>")
        .append(dimRatingNote(value(deliver, "ratingLogic")));
    sb.append("<h3>")
        .append(esc(value(histFin, "年份")))
        .append(" 历史财报验证</h3>")
        .append(
            cards(
                List.of(
    ...
    sb.append(
        cards(
            List.of(
                card("业绩兑现确定性", value(deliver, "业绩兑现确定性")), card("唯一变量", value(deliver, "唯一变量")))));
    ...
    sb.append("<h2>④ 瓶颈与壁垒 <span class='rating-tag'>")
        .append(esc(value(barrier, "rating")))
        .append("</span></h2>")
    ...
    sb.append("<div>")
        .append(esc(safeText(summary.get("oneLiner"), r.getVerdict(), "暂无可用结构化数据")))
        .append("</div></div>");
                        ...
                        joinLines(
                            List.of(
                                "区间最高: "
                                    + value(
                                        asMap(
                                            r.getNineDimension() == null
                                                ? null
                                                : r.getNineDimension().get("market")),
                                        "periodHigh"),
                                "区间最低: "
                                    + value(
                                        asMap(
  ...
  private String barrierListHtml(List<Map<String, Object>> items, String cssClass) {
    ...
    for (Map<String, Object> item : items) {
      ...
      sb.append("<div class='barrier-row ")
          .append(cssClass)
          .append("'>")
          .append("<div class='barrier-type'>")
          .append(esc(type))
          .append("</div>")
          .append("<div class='barrier-data'>")
          .append(esc(data))
          .append("</div>")
          .append("</div>");
    }
  ...
  private void appendStairRow(StringBuilder sb, String label, String value) {
    ...
    sb.append("<tr><th style='width:30%'>")
        .append(esc(label))
        .append("</th>")
        .append("<td>")
        .append(esc(value))
  ...
  private void appendForwardForecastTable(StringBuilder sb, List<Map<String, Object>> rows) {
    ...
    sb.append(
        "<table><thead><tr><th>年份</th><th>归母净利润</th><th>综合毛利率</th><th>核心兑现逻辑</th></tr></thead><tbody>");
    for (Map<String, Object> row : rows) {
      sb.append("<tr><td>")
          .append(esc(row.get("年份")))
          .append("</td>")
          .append("<td>")
          .append(esc(row.get("归母净利润")))
          .append("</td>")
          .append("<td>")
          .append(esc(row.get("综合毛利率")))
          .append("</td>")
          .append("<td>")
          .append(esc(row.get("核心兑现逻辑")))
          .append("</td></tr>");
    }
  ...
  public String normalizeCode(String code) {
    ...
    if (normalized.matches("\\d{6}")) {
      if (normalized.startsWith("60") || normalized.startsWith("68") || normalized.startsWith("90"))
        return "sh." + normalized;
      if (normalized.startsWith("00") || normalized.startsWith("30") || normalized.startsWith("20"))
        return "sz." + normalized;
      if (normalized.startsWith("43")
          || normalized.startsWith("83")
          || normalized.startsWith("87")
          || normalized.startsWith("88")) return "bj." + normalized;
    }
    return normalized;
  }

  private Map<String, Object> buildSourceMetadata(
      Map<String, Object> rawData,
    ...
    out.put(
        "db",
        sourceMeta(
            !dbFinancials.isEmpty(), !dbFinancials.isEmpty() ? "trade_stock_*" : "暂无可用结构化数据"));
    out.put(
        "baostock",
        sourceMeta(
            rawData != null && !rawData.isEmpty(),
            rawData != null && !rawData.isEmpty() ? "pack/financial_history/quote" : "暂无可用结构化数据"));
    boolean forecastAvailable =
        forecastSummary != null && forecastSummary.get("available") == Boolean.TRUE;
    out.put(
        "forecast",
        sourceMeta(
            forecastAvailable, forecastAvailable ? "baostock forecast + DB 估算" : "暂无可用结构化数据"));
    boolean searchAvailable =
        externalExpectation != null && !"暂无可用结构化数据".equals(externalExpectation.get("summary"));
    ...
    if (windResearch == null) {
      ...
      int consensusRows =
          windResearch.getConsensus() == null ? 0 : windResearch.getConsensus().getSourceRowCount();
  ...
  private Map<String, Object> buildForecastSummary(
      Map<String, Object> rawData, Map<String, Object> aiAnalysis, TradeStockBasic basic) {
    Map<String, Object> out = new LinkedHashMap<>();
    ...
    for (Object item : asList(rawData.get("forecast"))) {
      ...
      if (!forecast.isEmpty()) {
        ...
        normalized.put(
            "content",
            safeText(forecast.get("content"), forecast.get("desc"), forecast.toString()));
  ...
  private Map<String, Object> buildExternalExpectation(String stockName) {
    ...
    List<WebSearchClient.SearchResult> results =
        webSearchClient.search(stockName + " 机构预测 盈利预测 目标价");
    if (results.isEmpty()) {
      return out;
    }
    // 收集前 3 条搜索结果用于摘要
  ...
  private String summarizeExternalExpectation(
      String stockName, String rawText, WebSearchClient.SearchResult first) {
    if (rawText == null || rawText.isBlank()) return "暂无可用结构化数据";
    String clipped = rawText.length() > 2400 ? rawText.substring(0, 2400) : rawText;
    String sys =
        "你是中文金融研报助手，专注 A 股个股。把用户提供的英文/外文研究摘要翻译并压缩成 2-3 句中文结论，重点保留目标价、EPS 预测、关键业务驱动。\n"
            + "硬性要求：\n"
            + "1. 必须使用中文输出，不要夹杂英文原句。\n"
    ...
    String fallback =
        first == null
            ? ""
            : (first.getContent() != null && !first.getContent().isBlank()
                ? first.getContent()
                : first.getTitle());
    ...
    rows.stream()
        ...
        .forEach(
            row -> {
              ...
              item.put(
                  "reportDate",
                  row.getReportDate() == null ? null : row.getReportDate().toString());
  ...
  private Double latestPrice(String stockCode) {
    ...
    Map<String, AStockDataQuoteService.QuoteSnapshot> quotes =
        aStockDataQuoteService.fetchQuotes(List.of(stockCode));
    AStockDataQuoteService.QuoteSnapshot snapshot =
        quotes == null ? null : quotes.get(stockCode.trim().toUpperCase(Locale.ROOT));
  ...
  private void appendDbFinancialTable(StringBuilder sb, List<Map<String, Object>> rows) {
    ...
    sb.append(
        "<table><thead><tr><th>报告期</th><th>营收(亿)</th><th>净利润(亿)</th><th>EPS</th><th>ROE</th><th>毛利率</th><th>净利率</th><th>营收YoY</th><th>扣非YoY</th></tr></thead><tbody>");
    for (Map<String, Object> row : rows) {
      sb.append("<tr><td>")
          .append(esc(row.get("reportDate")))
          .append("</td>")
          .append("<td>")
          .append(esc(row.get("revenue")))
          .append("</td>")
          .append("<td>")
          .append(esc(row.get("netProfit")))
          .append("</td>")
          .append("<td>")
          .append(esc(row.get("eps")))
          .append("</td>")
          .append("<td>")
          .append(esc(row.get("roe")))
          .append("</td>")
          .append("<td>")
          .append(esc(row.get("grossMargin")))
          .append("</td>")
          .append("<td>")
          .append(esc(row.get("netMargin")))
          .append("</td>")
          .append("<td>")
          .append(esc(row.get("revenueYoy")))
          .append("</td>")
          .append("<td>")
          .append(esc(row.get("deductedNetProfitYoy")))
          .append("</td></tr>");
    }
    sb.append("</tbody></table>");
  }
  ...
  private void metricRow(StringBuilder sb, String label, List<Double> values) {
    ...
    for (Double value : values) {
      sb.append("<td>")
          .append(value == null ? "-" : String.format("%.2f%%", value * 100))
          .append("</td>");
    }
    sb.append("</tr>");
  }
  ...
  private String sourceCard(String label, Map<String, Object> meta) {
    ...
    return "<div class='source-item'><div class='source-label'>"
        + esc(label)
        + "</div><div class='source-value "
        + (available ? "ok" : "miss")
        + "'>"
        + (available ? "可用" : "缺失/占位")
        + "</div><div class='card-value'>"
        + esc(meta == null ? "暂无可用结构化数据" : meta.get("detail"))
        + "</div></div>";
  }

  private String cards(List<String> cards) {
    return "<div class='cards-grid'>"
        + cards.stream().filter(s -> !s.isBlank()).reduce("", String::concat)
        + "</div>";
  }
  ...
  private String summaryLine(StockAnalysisResponse response) {
    Map<String, Object> summary =
        asMap(response.getAnalysis() == null ? null : response.getAnalysis().get("summary"));
