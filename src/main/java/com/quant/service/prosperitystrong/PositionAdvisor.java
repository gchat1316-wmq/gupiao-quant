 ...
 *
 *   保守/中性/乐观估值 ← PE × 利润 或 PS × 营收
 *   左侧建仓价 = P_mid × 0.7
 *   右侧确认价 = max(P_mid × 0.85, MA20 假设)
 *   第一目标价 = P_mid
 *   第二目标价 = P_high
 *   止损价 = 当前价 × 0.85
 *   仓位比例按综合评分分档
 *
 * 当 P_mid 不可估时,以当前价为锚做简化估算(给出参考价位)。
 */
@Component
public class PositionAdvisor {
    ...
    public void advise(ProsperityPickDaily pick, BigDecimal latestPrice, BigDecimal peTtm) {
        ...
        BigDecimal pMid;
        if (peTtm != null && peTtm.compareTo(BigDecimal.valueOf(5)) > 0
        ...
        pick.setBuyRightPrice(pMid.multiply(BigDecimal.valueOf(0.95)).setScale(2, RoundingMode.HALF_UP));
        pick.setSellTarget1(pMid.setScale(2, RoundingMode.HALF_UP));
        pick.setSellTarget2(pHigh);
        pick.setStopLossPrice(latestPrice.multiply(BigDecimal.valueOf(0.85)).setScale(2, RoundingMode.HALF_UP));

        // 仓位分档
        double s = pick.getCombinedScore() == null ? 0 : pick.getCombinedScore().doubleValue();
        double core;
        double tactical;
        if (s >= 85) { core = 8; tactical = 4; }
        else if (s >= 70) { core = 6; tactical = 2; }
        else if (s >= 60) { core = 4; tactical = 1; }
        else { core = 0; tactical = 0; }
