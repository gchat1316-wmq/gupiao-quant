...
public class PracticalSelectController {
  ...
  /** 同步入口包一层超时：上游行情 502 或 DB 连接池被占满时，最迟 25s 内给用户友好提示， 而不是让 HTTP 连接挂死、HikariCP 雪崩升级。 */
  private PracticalSelectResponse analyzeWithTimeout(String keyword) {
    ...
    try {
      ...
      return PracticalSelectResponse.builder().matched(false).message("请求被中断，请重试").build();
      ...
      return PracticalSelectResponse.builder().matched(false).message("分析失败：" + msg).build();
    }
  }
  ...
  public void pdf(@PathVariable Long id, HttpServletResponse response) throws java.io.IOException {
    ...
    try {
      ...
      String displayName =
          (rec.getStockName() == null ? rec.getStockCode() : rec.getStockName())
              + "-"
              + rec.getStockCode()
              + "-实战选股.pdf";
