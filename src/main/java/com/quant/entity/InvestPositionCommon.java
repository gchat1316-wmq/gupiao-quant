...
/**
 * 三池持仓状态聚合表（invest_stock_pool / tech_ai_pool / potential_pool 共用）。 统一存储：告警阈值 + 持仓策略聚合 + 持仓策略参数。
 *
 * <p>主键为 (pool_type, stock_code) 组合，确保同一股票可出现在不同池中。 持仓流水（成交记录）暂存于各池独立的 *_position_fill 表，待后续统一。
 *
 * @see InvestStockPool
 * @see TechAiPool
 * @see PotentialPool
