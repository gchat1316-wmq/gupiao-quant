...
/**
 * JwtAuthFilter 单元测试。 覆盖：有效 token → 设置 SecurityContext、禁用用户 → 不注入、 无效/过期 token → 不拦截、anonymous 降级。
 */
...
class JwtAuthFilterTest {
  ...
  void validBearerTokenSetsAuthentication() throws Exception {
    ...
    assertThat(auth.getAuthorities().stream().anyMatch(a -> a.getAuthority().equals("ROLE_ADMIN")))
        .isTrue();
